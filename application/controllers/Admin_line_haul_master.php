...
class Admin_line_haul_master extends CI_Controller{
	...
	public function all_line_haul()
	{
        $data					= array();
		$data['allroutedata']	= $this->basic_operation_m->get_query_result_array('select * from tbl_line_hual_master');  
        $this->load->view('admin/line_haul/view_line_haul',$data); 
	}
	
	public function add_line_haul()
	{
		$data['message']="";	


		$data['route_detail']=$this->basic_operation_m->get_query_result('select * from tbl_line_hual_master_details left join city on city.id=tbl_line_hual_master_details.city ','');
		...
		if (isset($_POST['submit'])) 
		{
			...
			$reg_details=$this->basic_operation_m->get_table_row('tbl_line_hual_master',$whr_r);
			...
			else
			{
					...
					$inserted_id=$this->basic_operation_m->insert('tbl_line_hual_master',$r);
						...
						//if($i!=0)
						{
							...
							if($city_id[$i] <> 'multiselect-all')
							{
								 $r_detail= array(
								  ...
								  $result=$this->basic_operation_m->insert('tbl_line_hual_master_details',$r_detail);
							}
	...
	public function update_line_haul($id)
	{
		...
		if($id!="")
		{
			...
			$data['rgn']=$this->basic_operation_m->get_table_row('tbl_line_hual_master',$whr);	
			$whr1 =array('routeid'=>$id);
			$data['rgn_detail']=$this->basic_operation_m->get_query_result("select * from tbl_line_hual_master_details left join city on city.id=tbl_line_hual_master_details.city where routeid='$id'",$whr1);
		...
		if (isset($_POST['submit'])) {
	        ...
	        $whr1 =array('routeid'=>$id);
	        $result=$this->basic_operation_m->delete('tbl_line_hual_master_details',$whr1);
			...
			$result=$this->basic_operation_m->update('tbl_line_hual_master',$r, $whr);
			if(!empty($r))
			{				
		...
		$data['city_list']= $this->basic_operation_m->get_all_result('city');
		$data['state_list']= $this->basic_operation_m->get_all_result('state');
	    $this->load->view('admin/line_haul/edit_line_haul',$data);
	}
	
	public function delete_line_haul($id)
	{
		$data['message']="";       
		if($id!="")
		{
		    $whr =array('route_id'=>$id);
			$this->basic_operation_m->delete('tbl_line_hual_master',$whr);	

			$whr1=array('routeid'=>$id);
			$this->basic_operation_m->delete('tbl_line_hual_master_details',$whr1);			
            redirect('admin/line-haul');
		}		
	...
	{  
		...
		$this->load->view('admin/line_haul/report_branch',$data);
	...
	{  
        ...
        $this->load->view('admin/line_haul/collection',$data);
	...
	{  
        ...
        $this->load->view('admin/line_haul/pettycash',$data);
...
,sum(grand_total)  AS Total,GROUP_CONCAT(booking_id) AS booking_ids FROM tbl_domestic_booking WHERE dispatch_details = 'Cash' AND branch_id = '$branch_id' and petty_cash_status ='0'  AND DATE(booking_date) <= CURDATE()");
	...
	{  
        ...
        $this->load->view('admin/line_haul/duereport',$data);
	...
	{  
		...
		//$data['city']  = $this->db->query('select city from city')->result_array();
        $this->load->view('admin/line_haul/branchcollection',$data);
