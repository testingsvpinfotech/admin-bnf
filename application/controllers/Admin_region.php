...
public function getStatewiseCity()
	{
		...
		if($state_id!="")
		{
			...
			// $res=$this->basic_operation_m->get_query_result("select city.id,city.state_id,city.city from city left JOIN region_master_details ON region_master_details.city = city.id where $whr AND rgm_id IS null order by city.city asc");
			...
			if(!empty($res))
			{
				...
				{   
					$zone_exists = $this->db->query("SELECT * FROM region_master_details WHERE state = '$values->state_id' AND city ='$values->id'")->row('regionid');
					if(empty($zone_exists))
					{
		...
		if($state_id!="")
		{
			...
			if(!empty($res))
			{
				...
				{   
					...
					$zone_exists = $this->db->query("SELECT * FROM region_master_details WHERE state = '$state_id' AND city ='$city_id'")->row('regionid');
					if(empty($zone_exists))
					{
						$html  .= '<option value="'.$values['state_id'].'_'.$values['id'].'"   >'.$values['city'].'</option>';
					 //option += '<option value="' +d[i].id + '">' + d[i].city + '</option>';
