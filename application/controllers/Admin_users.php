...
class Admin_users  extends CI_Controller {
	...
	{	
        $data = [];     
        $data['all_users']=$this->basic_operation_m->getAlldeletedUsers();         
        $this->load->view('admin/user_management/view_deleted_users', $data);
	}
            ...
            $whr =array('username'=>$this->input->post('username'));
         	...
         	if(!empty($check_available))
         	{
        		...
        		$class			= 'alert alert-danger alert-dismissible';	
         		...
         		$as_branch = implode(',', $this->input->post('assign_branch'));
				...
				$this->basic_operation_m->addLog($this->session->userdata("userId"),'operation','Add User', $data);
		...
		foreach($selected_menu as $key => $values)
		{
			$data['selected_menu'][] = $values->am_id;
          	...
          	if(isset($_POST['submit']))
          	{
				...
				$this->basic_operation_m->addLog($this->session->userdata("userId"),'operation','User Update', $data, $data['singleuser']);
            	...
            	// redirect('admin/list-user');   staf profile update iuess login
            	redirect('admin/list-user');
...
// 	    $id	= $this->uri->segment($last);
		if($id!=""){
			...
			$result=$this->basic_operation_m->update('tbl_users',$data, $whr );
			$this->basic_operation_m->addLog($this->session->userdata("userId"),'operation','Delete User Login Or Operation Disable', $data,$data['singleuser']);
	...
	public function delete_permanently_user()
	{
		$id = $this->input->post('getid');
				...
				if($id!=""){
			        ...
			        $this->basic_operation_m->addLog($this->session->userdata("userId"),'operation','Delete User Permanently',$data['singleuser']);
					$output['status'] = 'success';
					$output['message'] = 'User deleted successfully';
				}
				else{
