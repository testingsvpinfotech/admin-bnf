...
class User_panel extends CI_Controller {
       ...
       if(isset($_POST['submit']))
       {
            ...
            $customer_id=$this->session->userdata("customer_id");
       		...
       		if($company_type!="ALL")
       		{
			    ...
			    {	
               		...
               		$resAct2 = $this->db->query("select tbl_domestic_booking.*, city.city ,tbl_domestic_weight_details.no_of_pack,tbl_domestic_weight_details.actual_weight, tbl_domestic_weight_details.chargable_weight from tbl_domestic_booking LEFT JOIN tbl_domestic_weight_details ON tbl_domestic_weight_details.booking_id = tbl_domestic_booking.booking_id  LEFT JOIN `tbl_customers` ON `tbl_customers`.`customer_id` = `tbl_domestic_booking`.`customer_id` LEFT JOIN `city` ON `city`.`id` = `tbl_domestic_booking`.`reciever_city` WHERE tbl_domestic_booking.customer_id='$customer_id' and booking_date between '$from_date' and '$to_date' AND $d_status_fileter ORDER BY booking_date DESC");
               		...
               		if($resAct2->num_rows()>0)
               		{
               			...
               			foreach($temp2 as $t2)
               			{
        					...
        					if($resAct2->num_rows()>0){
        						$status = @$resAct2->row_array();
								$status_data =	$status['status'];
								$branch_name =	$status['branch_name'];
        					    $row_d=array('booking_date'=>$t2->booking_date,'pod_no'=>$t2->pod_no,'no_of_pack'=>$t2->no_of_pack,'actual_weight'=>$t2->actual_weight,'chargable_weight'=>$t2->chargable_weight,'sender_name'=>$t2->sender_name,'doc_nondoc'=>$t2->doc_nondoc,'city'=>$t2->city,'forwording_no'=>$t2->forwording_no,'forworder_name'=>$t2->forworder_name,'reciever_name'=>$t2->reciever_name,'invoice_no'=>$t2->invoice_no,'invoice_value'=>$t2->invoice_value,'status'=>$status_data,'branch_name'=>$branch_name);
			...
			else
			{
               		...
               		if($status !="ALL")
					{
               		...
               		$resAct_d = $this->db->query("select tbl_domestic_booking.*, city.city ,tbl_domestic_weight_details.no_of_pack,tbl_domestic_weight_details.actual_weight, tbl_domestic_weight_details.chargable_weight from tbl_domestic_booking LEFT JOIN tbl_domestic_weight_details ON tbl_domestic_weight_details.booking_id = tbl_domestic_booking.booking_id LEFT JOIN `tbl_customers` ON `tbl_customers`.`customer_id` = `tbl_domestic_booking`.`customer_id` LEFT JOIN `city` ON `city`.`id` = `tbl_domestic_booking`.`reciever_city` WHERE tbl_domestic_booking.customer_id='$customer_id' and booking_date between '$from_date' and '$to_date' AND $d_status_fileter ORDER BY booking_date DESC");
	...
	{       
		...
		//echo $from_date.$to_date.$status.$company_type;
       			...
       			foreach($temp2 as $t2)
       			{
					...
					if($resAct2->num_rows()>0){
						...
						$status = @$resAct2->row_array();
								...
								$status_data =	$status['status'];
								$branch_name =	$status['branch_name'];
					    $row=array('booking_date'=>$t2->booking_date,'pod_no'=>$t2->pod_no,'no_of_pack'=>$t2->no_of_pack,'actual_weight'=>$t2->actual_weight,'chargable_weight'=>$t2->chargable_weight,'sender_name'=>$t2->sender_name,'doc_nondoc'=>$t2->doc_nondoc,'city'=>$t2->city,'forwording_no'=>$t2->forwording_no,'forworder_name'=>$t2->forworder_name,'reciever_name'=>$t2->reciever_name,'invoice_no'=>$t2->invoice_no,'invoice_value'=>$t2->invoice_value,'status'=>$status_data,'branch_name'=>$branch_name);
			...
			$header =array("SrNo.","Date","AWB.","Destination","Receiver","Doc/Non-doc","Invoice No","Invoice Value","no_of_pack","actual weight","chargable weight","Status");
			...
			foreach($data['alldata'] as $value) {
				...
				$row=array($i,$booking_date,$value_d['pod_no'],$value_d['city'],$value_d['reciever_name'],$value_d['doc_nondoc'],$value_d['invoice_no'],$value_d['invoice_value'],$value_d['no_of_pack'],$value_d['actual_weight'],$value_d['chargable_weight'],$value_d['status']);
				fputcsv($fp, $row);
			}
	...
	public function list_domestic_shipment($offset=0,$searching='')
	{
		...
		$resAct = $this->db->query("SELECT tbl_domestic_booking.*,city.city,tbl_domestic_weight_details.actual_weight,tbl_domestic_weight_details.chargable_weight,payment_method  FROM tbl_domestic_booking LEFT JOIN city ON tbl_domestic_booking.reciever_city = city.id  LEFT JOIN tbl_domestic_weight_details ON tbl_domestic_weight_details.booking_id = tbl_domestic_booking.booking_id  WHERE booking_type = 1 $where  order by tbl_domestic_booking.booking_id DESC limit ".$offset.",50");
	...
	public function view_multipale_pod_download()
	{   
		...
		{	
			...
			foreach($all_data as $ke=> $vall)
			{
				...
				// 	if($vall == 'pod_no')
				// 	{
				// 		$filterCond .= " AND tbl_upload_pod.pod_no = '$filter_value'";
				// 	}					
				// }
				if($ke == 'from_date' && !empty($vall))
				{
					$filterCond .= " AND tbl_upload_pod.booking_date >= '$vall'";
				}
			 ...
			 $data['pod'] = $this->db->query("select tbl_upload_pod.* from tbl_upload_pod JOIN tbl_domestic_booking on tbl_domestic_booking.pod_no = tbl_upload_pod.pod_no where tbl_domestic_booking.customer_id = '$customer_id' $filterCond")->result();
			//  echo $this->db->last_query();die;
			//  echo '<pre>'; print_r($data['pod']);die;
		}
		$data['message']="";
	    $this->load->view('user/pod/download_pod',$data);
	}
	...
	public function pickup_request()
	{
          ...
          //  print_r($count);exit;
			...
			$r1 = array(
				...
				if(!empty($pickup_pincode)){
					...
					$branch_pincode = $this->db->query("select * from tbl_branch where pincode = '$pin_code'")->row_array();
			...
			redirect('User_panel/pickup_request');
		}
		...
		$this->load->view('user/add_pickup_request',$data);
	...
	public function add_new_rate_domestic()
	{
		...
		$fixed_perkg_result = $this->db->query("select * from tbl_domestic_rate_master where 
			...
			AND (from_state_id=" . $sender_state . " OR state_id=0)
	...
	public function insert_domestic_shipment()
	{
		...
		if (!empty($all_Data)) {
			...
			$data = array(
				...
				'booking_time' => date('H:i:s', strtotime($this->input->post('booking_date'))),
				'dispatch_details' => $this->input->post('dispatch_details'),
				'payment_method' => !empty($this->input->post('payment_method'))?$this->input->post('payment_method'):'0',
				'frieht' => $this->input->post('frieht'),
				'transportation_charges' => $this->input->post('transportation_charges'),
				'pickup_charges' => $this->input->post('pickup_charges'),
				'delivery_charges' => $this->input->post('delivery_charges'),
			...
			if (empty($lastid)) {
				...
				$this->basic_operation_m->insert('tbl_domestic_stock_history', $stock32);
				if ($this->input->post('customer_account_id') != "") {
					$whr = array('customer_id' => $customerid);
					$res = $this->basic_operation_m->getAll('tbl_customers', $whr);
					$email = $res->row()->email;
				...
				$this->Booking_notifection($lastid);
			}
...
public function check_rate()
	{
		...
		$fixed_perkg_result = $this->db->query("select * from tbl_domestic_rate_master where 
			...
			AND DATE(`applicable_from`)<='$current_date'
			AND DATE(`applicable_to`)>='$current_date'
			AND ($chargable_weight1
			BETWEEN weight_range_from AND weight_range_to)  
			ORDER BY state_id DESC,city_id DESC,customer_id DESC,applicable_from DESC LIMIT 1");
		$values = $fixed_perkg_result->row();
		// echo $this->db->last_query();die;
		if (!empty($values)) {
			if ($values->fixed_perkg == 4 && ($this->input->post('chargable_weight') >= $values->weight_range_from && $this->input->post('chargable_weight') <= $values->weight_range_to)) {
				$weight = ceil($actual_weight);
			} else {
				$weight = round($actual_weight);
			}
		} else {
			$weight = round($actual_weight);
		}
	...
	public function Booking_notifection($booking_id){

		$booking_info = $this->basic_operation_m->get_table_row('tbl_domestic_booking', ['booking_id'=>$booking_id]);

		$branch_id = $this->basic_operation_m->get_table_row('tbl_branch', ['branch_id'=>$booking_info->branch_id]);
		$customer_id = $booking_info->customer_id;
		$customer_info = $this->basic_operation_m->get_table_row('tbl_customers', ['customer_id'=>$customer_id]);
		$sales_person_id = $customer_info->sales_person_id;
		$sales_person = $this->basic_operation_m->get_table_row('tbl_users', ['user_id'=>$sales_person_id]);
	
		$weghit = $this->basic_operation_m->get_table_row('tbl_domestic_weight_details', ['booking_id'=>$booking_id]);
		$origin = $this->basic_operation_m->get_table_row('city', ['id'=>$booking_info->sender_city]);
		$destination = $this->basic_operation_m->get_table_row('city', ['id'=>$booking_info->reciever_city]);
		$email_body = "";
		$email_body 	="<p>Dear Sir/Madam,</p>";

		$email_body 	.="<p>I hope you're having a wonderful day </p> <p> I am ".$customer_info->customer_name." ( ".$customer_info->cid." ) We have booked some shipments requesting to you please pickup them shipment I am providing required information </p>";

		$email_body 	.='<p> If you need any further details please contact</p>
		<br><br><table style="width:100%;border: 2px solid black;">
		<thead>
			<tr style="border: 1px solid black; border-collapse: collapse;">
				<th style="border: 1px solid black; border-collapse: collapse;">Date</th>
				<th style="border: 1px solid black; border-collapse: collapse;">AWB No.</th>
				...
				<th style="border: 1px solid black; border-collapse: collapse;">Chargable Weight</th>
				<th style="border: 1px solid black; border-collapse: collapse;">NOP</th>
				<th style="border: 1px solid black; border-collapse: collapse;">Invoice No.</th>
				<th style="border: 1px solid black; border-collapse: collapse;">Invoice Value</th>

			</tr>
		</thead><tbody style="border: 1px solid black; border-collapse: collapse;">
             <tr>
			 <td style="border: 1px solid black; border-collapse: collapse; text-align:center;">'.date('d-m-Y',strtotime($booking_info->booking_date)).'</td>
			 <td style="border: 1px solid black; border-collapse: collapse; text-align:center;">'.$booking_info->pod_no.'</td>
			 <td style="border: 1px solid black; border-collapse: collapse; text-align:center;">'.$origin->city.'</td>
			 <td style="border: 1px solid black; border-collapse: collapse; text-align:center;">'.$destination->city.'</td>
			 <td style="border: 1px solid black; border-collapse: collapse; text-align:center;">'.$weghit->chargable_weight.'</td>
			 <td style="border: 1px solid black; border-collapse: collapse; text-align:center;">'.$weghit->no_of_pack.'</td>
			 <td style="border: 1px solid black; border-collapse: collapse; text-align:center;">'.$booking_info->invoice_no.'</td>
			 <td style="border: 1px solid black; border-collapse: collapse; text-align:center;">'.$booking_info->invoice_value.'</td>
			 </tr>
		</tbody>
		</table>';
		
		$email_body .= '<br><br><b>Thanks & Regards,<br> '.$customer_info->customer_name.'</b><br><b>Email : </b>'.$customer_info->email.'<br><b>Phone : </b>'.$customer_info->phone;
			...
			// $email = ["mobile.svpinfotech@gmail.com","manoj.s@boxnfreight.com","testing.svpinfotech@gmail.com"];
			$email = ["mobile.svpinfotech@gmail.com","manoj.s@boxnfreight.com",$sales_person->email];
			$config['newline'] = "\r\n";

			$this->email->clear(TRUE);
			$subject = 'Customer Booking Shipment Pickup Request';
			...
			$this->email->from('noreply@boxnfreight.in', $customer_info->customer_name." ( ".$customer_info->cid." )");
			$this->email->to($branch_id->email); 
			// $this->email->to("seo.svpinfotech@gmail.com"); 
			$this->email->cc($email); 
			//$this->email->cc('manoj.s@boxnfreight.com'); //$customer_info->email
			// $this->email->cc('pankaj.g@boxnfreight.com'); //$customer_info->email
			$this->email->subject($subject);
			$this->email->message($email_body);  
			// $this->email->attach($filename);
			if ($this->email->send()) {
				$msg = 'Your Shipment ' . $booking_info->pod_no . ' status:Boked  At Location: ' .$branch_id->branch_name;
	...
	public function getsenderdetails()
	{
		...
		foreach ($state_list as $value) {
			...
			$state .= '<option value="' . $value["id"] . '" ' . $selected . ' >' . $value["state"] . '</option>';
		}
		$pincode_city =  $res1->row('city');
	   ...
	   public function getVehicleCapicty()
	   {
		 ...
		 // echo $this->db->last_query();
		  echo json_encode($data);
			...
			{
				$customer_id = $this->session->userdata('customer_id');
				$ftl_list = $this->db->query("SELECT ftl_request_tbl.* ,vehicle_type_master.vehicle_name,ftl_document_image_tbl.eway_image  FROM ftl_request_tbl left join vehicle_type_master ON vehicle_type_master.id = ftl_request_tbl.type_of_vehicle  left join ftl_document_image_tbl ON ftl_document_image_tbl.ftl_id = ftl_request_tbl.ftl_request_id  where  customer_id = '$customer_id' order by ftl_request_id  DESC limit ".$offset.",10");
