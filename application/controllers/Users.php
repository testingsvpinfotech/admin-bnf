...
class Users extends CI_Controller {
	...
	public function pod_download($file){
      force_download('assets/pod/'.$file, NULL);
     exit;
	}
...
// 					$reAct=$this->db->query("select * from tbl_domestic_tracking LEFT JOIN tbl_branch ON tbl_branch.`branch_name`=tbl_domestic_tracking.branch_name  
// LEFT JOIN city on city.id=tbl_branch.city  where  pod_no = '$pod_no' ORDER BY tbl_domestic_tracking.id DESC;");
// 					$data['pod']		=	$reAct->result();
// 					$data['del_status']	=	$reAct->row();
...
public function track_shipment()
	{
		...
		if (isset($_GET['submit']))
        {
			$pod_no							=	trim($this->input->get('pod_no'));  
			$check_pod_international		=	$this->db->query("select pod_no from tbl_international_booking where pod_no = '$pod_no'");
			$check_result					=	$check_pod_international->row();
			...
			else
			{
				...
				$reAct=$this->db->query("select tbl_domestic_booking.*,tbl_domestic_weight_details.no_of_pack, sendercity.city AS sender_city_name, recievercity.city as reciever_country_name from tbl_domestic_booking left join tbl_domestic_weight_details on tbl_domestic_booking.booking_id=tbl_domestic_weight_details.booking_id INNER JOIN city sendercity ON sendercity.id = tbl_domestic_booking.sender_city INNER JOIN city recievercity ON recievercity.id = tbl_domestic_booking.reciever_city where pod_no = '$pod_no' and tbl_domestic_booking.pickup_in_scan != '2' and tbl_domestic_booking.branch_in_scan != '2'");
				$data['info']					=	$reAct->row();
				...
				if(!empty($data['info']))
				{
					...
					// $reAct=$this->db->query("select * from tbl_domestic_tracking WHERE pod_no='$pod_no' AND status <> 'getpass-genrated' AND status <> 'getpass-in-scan' ORDER BY id DESC");
