...
class Rate_model extends CI_Model
{
	...
	public function insert_domestic_rate($tablename, $data_value, $customer_id, $transfer_date)
	{
		foreach  ($customer_id as $to_customer_id) {
			foreach ($data_value as $key => $data) {			
					...
					$data1 = array(
						...
						'applicable_from' => date("Y-m-d", strtotime($transfer_date)),
						'applicable_to' => date("Y-m-d", strtotime($data['applicable_to'])),
						'weight_range_from' => $data['weight_range_from'],
						'weight_range_to' => $data['weight_range_to'],
						'weight_slab' => $data['weight_slab'],
						'rate' => $data['rate'],
						'fixed_perkg' => $data['fixed_perkg']
