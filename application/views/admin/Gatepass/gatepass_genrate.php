     					...
     					<div class="col-12 col-sm-12 mt-3">
     						<div class="card">
     							<div class="card-header justify-content-between align-items-center">
     								<h4 class="card-title">Genrate Master Menifest</h4>
     							</div>
     							<div class="card-body">
     								...
     								<div class="row">
     									<div class="col-12"> <a href="<?= base_url('admin/view-genrated-gatepass'); ?>" class="btn btn-primary">View Genrated Master Menifest</a> <br><br>
     										<form role="form" action="<?= base_url(); ?>admin/gatepass" method="post" enctype="multipart/form-data">
     											<div class="form-group row" id="access_or_not" >
												 <label class="col-sm-1 col-form-label">Menifest No</label>
     												<div class="col-sm-2">
     													<input type="text" name="menifest" id="menifest" class="form-control" />
     												</div>
     												<label class="col-sm-1 col-form-label" style="display:none;">Bag Scan</label>
     												<div class="col-sm-2" style="display:none;">
     													<input type="text" name="bag_scan" id="awb" class="form-control" />
     												</div>
     												<div class="col-sm-2">
     										...
     										<form role="form" action="<?= base_url('admin/add-gatepass'); ?>" method="post" enctype="multipart/form-data">
     											<div class="form-group row">

     												<label class="col-sm-2 col-form-label">Master Menifest Genrate Date</label>
     												<div class="col-sm-2">
     													...
     													<?php
															...
															// $datec = str_replace(' ', 'T', $datec);
															?>
     													<input type="datetime-local" required class="form-control" name="datetime" value="<?php echo $datec; ?>"  id="mastermenifest_back" min="<?= date('Y-m-d', strtotime("-1 days")).'T'.date('H:i'); ?>" max="<?= date('Y-m-d').'T'.date('H:i'); ?>">
     												</div>
     												...
     												<div class="col-sm-2">
     													<input type="text" name="driver_name" required  readonly class="form-control" value="<?php if(! empty($result)){ echo $result[0]->driver_name;} ?>"/>
     												</div>
     											</div>
     											<div class="form-group row">
												    <label class="col-sm-2 col-form-label">Vehicle No</label>
     												<div class="col-sm-2">
     													<input type="text" name="vehicle_no" required readonly class="form-control" value="<?php if(! empty($result)){ echo $result[0]->lorry_no;} ?>"/>
     												</div>
												    <label class="col-sm-2 col-form-label">Origin</label>
     												<div class="col-sm-2">
     													<input type="text" name="origin" required readonly class="form-control" value="<?php if(! empty($result)){ echo $result[0]->source_branch;} ?>"/>
     												</div>
												    <label class="col-sm-2 col-form-label">Destination</label>
     												<div class="col-sm-2">
     													<input type="text" name="destination" required readonly class="form-control" value="<?php if(! empty($result)){ echo $result[0]->destination_branch;} ?>"/>
     												</div>
     											</div>
												<div class="form-group row">
												    <label class="col-sm-2 col-form-label">Lock No</label>
     												<div class="col-sm-2">
     													<input type="text" name="lock_no"  class="form-control" />
     												</div>
     												...
     												<table class="table table-bordered table-striped">
     													...
     													<tbody>
															<?php $username = $this->session->userdata("userName");
																...
																$source_branch		= 	$ress->row()->branch_name;  if($result){ $count = 1;  foreach ($result as $key=>$value) { ?>
																<tr>
																	<!-- <td><?= $count++; ?></td> -->
																	<td><?php echo $value->date_added;?> <input type="hidden" id="custId" name="manifiest_id" value="<?php echo $value->manifiest_id;?>"></td>
																	<td><?php echo $value->manifiest_id;?> 
																	<td><?php echo $value->bag_no;?> 
																	<td><?php $whr3 = array('bag_id'=>$value->bag_no,'source_branch'=> $source_branch);
		                                                         	$ress = $this->basic_operation_m->getAll('tbl_domestic_bag', $whr3); $count = $ress->result(); echo count($count);?></td>
																	<td><?php echo $value->source_branch;?></td>
																	<td><?php echo $value->destination_branch;?></td>
																	<td><?php echo $value->username;?></td>
																	<td><?php echo $value->supervisor;?> <input type="hidden" id="custId" name="bag" value="<?php echo $value->bag_no;?>">
																</tr>
															<?php }} ?>
     													</tbody>
