...
<style>
  ...
  <?php  unset($_SESSION['class']); unset($_SESSION['notify']); } ?>       
						   <div class="row">                                           
                            <div class="col-12">
								<form role="form" action="<?php echo base_url('admin/update-menifiest/'.$menifiest_info->id)?>" method="post" enctype="multipart/form-data">
								<!-- <form role="form" action="admin/update-menifiest/"<?php // echo $menifiest_info->id; ?> method="post" enctype="multipart/form-data"> -->

								<div class="form-group row">
									<label  class="col-sm-2 col-form-label">Manifiest Id</label>
									<div class="col-sm-2">
										<input type="text" class="form-control" name="manifiest_id" id="col-sm-1 col-form-label" placeholder="Enter Id" readonly value="<?php echo $menifiest_info->manifiest_id; ?>" >
									...
									<div class="col-sm-2">
										<input type="date" class="form-control" readonly name="datetime" id="col-sm-1 col-form-label"  value="<?php echo $menifiest_info->date_added; ?>">
									</div>
								...
								<div class="form-group row">																
									...
									// $franchise1 = $this->db->query("select * from tbl_customers where branch_id = '$branch_id' AND (customer_type ='1' OR customer_type ='2') ")->result();
									...
									<label  class="col-sm-2 col-form-label">CD No <input type="datetime-local" class="form-control" name="cd_no_edited_date" required   min="<?= date('Y-m-d', strtotime("-1 days")).'T'.date('H:i'); ?>" max="<?= date('Y-m-d').'T'.date('H:i'); ?>"></label>
									<div class="col-sm-2">
										<input type="text" name="cd_no" class="form-control" value="<?php echo $menifiest_info->cd_no; ?>"  />
									</div>
									<label  class="col-sm-2 col-form-label">Destination Branch</label>
									<div class="col-sm-2">
										<select name="destination_branch" class="form-control" id="destination_branch" required disabled>
											...
											<option value = "<?= $val->customer_id; ?>" <?php echo ($val->customer_id == $menifiest_info->destination_branch)?'selected':''; ?>>To <?= " ".$val->customer_name."_".$val->cmp_area;?> Franchise</option>
										  <?php } ?>
										</select>
									</div>
...
<!-- END: Body-->
        ...
        <script type="text/javascript">
          $(document).ready(function()
		  {
			...
			// $("#datepicker").datepicker('option', {minDate: "-1D", maxDate: "+1D"});
