					...
					<div class="col-12 col-sm-12 mt-3">
						<div class="card">
							<div class="card-header justify-content-between align-items-center">
								<h4 class="card-title">Add Franchise Incoming Bag</h4>
							</div>
							<div class="card-body">

								<div class="row">
									<div class="col-12">
										<form role="form" action="admin/add-franchise-incoming-bag" method="post">
											<div class="box-body">
															...
															if (!empty($menifiest)) {
																foreach ($menifiest as $row) {
																	...
																	if ($row->bag_recived == 0) {
																		?>
																		<option value="<?php echo $row->bag_no; ?>" <?php echo ($manifiest_id == $row->bag_no) ? 'selected' : ''; ?>>
																			<?php echo $row->bag_no; ?> --
										...
										<div class="form-group row">
											...
											<div class="col-sm-2">
												<input type="datetime-local" class="form-control" name="datetime"
													...
													value="<?php echo Date('Y-m-d\TH:i', time()) ?>" readonly>
											</div>
										</div>
										<thead>
											<tr>
										...
										<tbody>
											...
											if (!empty($menifiest_data)) {
												foreach ($menifiest_data as $value) {
													...
													?>
													<tr>
														<td><input type="checkbox" name="pod_no[]" <?php echo ($value->bag_recived > 0) ? 'disabled' : ''; ?> checked value="<?= $value->pod_no ?>"></td>
														<td><?= $value->pod_no ?></td>
														<!-- <td><?= $value->destination_branch ?></td> -->
														<td><?= $value->source_branch ?></td>
														<td><?= $value->total_pcs ?></td>
														<td><?= $value->total_weight ?></td>
														<td><textarea name="recived_nop" <?php echo ($value->bag_recived > 0) ? 'disabled' : ''; ?> placeholder="Recived NOP"></textarea>
														</td>
														<td><textarea name="balance_nop" <?php echo ($value->bag_recived > 0) ? 'disabled' : ''; ?> placeholder="Balance NOP"></textarea>
														</td>
														<?php
														if ($value->bag_recived == 0) { ?>
															<td id="<?php echo $value->pod_no; ?>">
																<?php echo ($value->bag_recived > 0) ? 'Recieved' : ''; ?>
															</td>
															<input type="hidden" name="value_<?php echo $value->pod_no; ?>"
																id="value_<?php echo $value->pod_no; ?>" value="">
														<?php
														} else {
															$recived++;
															?>
															...
															<td>Recieved</td>
														<?php } ?>
														<td><textarea name="note[]" <?php echo ($value->bag_recived > 0) ? 'disabled' : ''; ?> placeholder="remark"></textarea></td>
