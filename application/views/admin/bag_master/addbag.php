                            ...
                            <div class="col-12">
								...
								<div class="form-group row" >
									...
									$datec = date('Y-m-d H:i');
										...
										<input type="datetime-local" readonly required class="form-control" name="datetime" value="<?php echo $datec;?>" id="booking_bg" min="<?= date('Y-m-d', strtotime("-1 days")).'T'.date('H:i'); ?>" max="<?= date('Y-m-d').'T'.date('H:i'); ?>">
									</div>
									...
									<div class="col-sm-2">
										<select name="forwarder_name" class="form-control" id="forwarderName" required>
											...
											<?php foreach ($courier_company as $value) {
												?>
												<option value="<?php echo $value['c_company_name'];?>" <?php if($value['c_company_name'] == 'SELF'){ echo 'selected' ;} ?>><?php echo $value['c_company_name'];?></option>
												<?php 
											} ?>
										</select>
									</div>
								...
								<div class="form-group row">
									...
									<!-- <div class="form-group col-sm-6" id="bkdate_bag_check"> -->
									<!-- <div class="form-group col-sm-6"> -->
												<label  class="col-sm-2 col-form-label">Comment</label>
												<div class="col-sm-2">
													<textarea type="text" name="bkdate_reason" id="bkdate_bag_reason" class="form-control" value="<?php //echo $bid; ?>"></textarea>
												</div>
											<!-- </div> -->
...
$('#bkdate_bag_check').css({"display":"none"});
	$("#booking_bg").change(function(){
		let dt = $("#booking_bg").val();
		var bdt = new Date(dt);
		...
		if(output == boutput){
			...
			$("#bkdate_bag_reason").removeAttr("required");
			$('#bkdate_bag_check').css({"display":"none"});
		}else{
			$("#bkdate_bag_reason").attr("required", "true");
			$('#bkdate_bag_check').css({"display":"flex"});
		}
	});
