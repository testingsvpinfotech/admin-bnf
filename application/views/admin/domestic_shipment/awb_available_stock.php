        ...
        <div class="col-12">
            ...
            <div class="card-body">
                ...
                <?php unset($_SESSION['class']);
                    ...
                    <table  class="display table  table-responsive table-striped table-bordered">
                            <thead>
                            <tr>                 
                                ...
                                <th>Utilized are marked in red : <span style="color:red;"><?='00'; ?></span></th>
                            </tr>
                            </thead>
                    </table> <br>
                    <form action="<?= base_url('admin/stock-customer');?>" method="get" enctype="multipart/form-data">
                            <div class="row">
                            ...
                            <select class="form-control" name="customer_id"
												...
												<?php
                                                $customers = $this->db->query("SELECT * FROM tbl_customers WHERE customer_type !=1 and customer_type !=2")->result_array();
												if (count($customers)) {
													foreach ($customers as $rows) {
														?>
														<option value="<?php echo $rows['customer_id']; ?>" <?php if($rows['customer_id']==$_GET['customer_id']){echo 'selected';}?>>
                            ...
                            <a href="<?= base_url('admin/stock-customer');?>" class="btn btn-primary"><i class="fa fa-refresh" style="font-size:14px;"></i></a>
                            </div>
                        </div>
                        </form>
                        <br>
...
<script>
        ...
        $(".booking").click(function () 
	{
      ...
      $.ajax({
        type: 'POST',
        url: '<?php echo base_url();?>Admin_customer/get_booking_info',
        data: {pod_no:pod_no},
        dataType: "html",
        success: function (d) {     
