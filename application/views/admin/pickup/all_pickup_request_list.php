                                    ...
                                    <div style="margin:15px;">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-4">
                                                    <div class="form-group">
                                                        <label>Date</label> 
                                                        <input type="datetime-local" class="form-control" name="pickup_boy_assigned_date" id="pickup_boy_assigned_date" required>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    <div class="col-md-1">
                                                        <button class="btn btn-primary mt-4 pickup_boy_assigned" data-url="Pickup_Request_Controller/prq_assign_pickupboy" type="button">Assign</button>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <table class="table layout-primary table-bordered">
                                            <thead>
                                                <tr style="background-color: #ef7f1a;">
                                                    <td>PRQ Reschedule</td>
                                                    <?php if($user_type != '16' || $user_type != '1'){?>
                                                    <td>Pickup Boy Assign</td>
                                                    <?php } ?>
                                                    <!-- <td></td> -->
                                            ...
                                            <tbody>
                                                ...
                                                ?>
                                                        <tr class="odd gradeX">
                                                            ...
                                                            <?php if($ct['branch_id'] != '0' && $ct['pickup_status'] != '1'){?>
                                                            <td class="text-primary reschedul_prq"  data-id = "<?php echo $ct['pickup_request_id']; ?>">PRQ Reschedule</td>
                                                            <?php } elseif($ct['pickup_status'] == '1') { ?>
                                                               <td></td>
                                                            <?php }else{?>
                                                            ...
                                                            <?php } ?>
                                                             <!-- //assign pickup boy -->
                                                             <?php if($user_type != '16' || $user_type != '1'){?>
                                                            <?php if(empty($ct['pickup_boy']) || $ct['pickup_status'] != '1'){ ?>
                                                                <td><input type="checkbox" class="check_assign_prq" data-id="<?php echo $ct['id']; ?>"></td>
                                                           <?php }else{?>
                                                            <td></td>
                                                          <?php } } ?>
                                                            ...
                                                            <td>
                                                               <?php $pqno = $ct['pickup_request_id']; $d = $this->db->query("select pod_no from tbl_domestic_booking where prq_no = '$pqno'")->result_array();?>
                                                                <?php  if(!empty($d)){?>
                                                                    <span class="show_docket_data" data-id="<?php echo $ct['pickup_request_id']; ?>" style="color:#e1383b">View Docket</span>
                                                            ...
                                                            <?php if ($user_type != '16') { ?>
                                                                ...
                                                                <td >
                                                                    ...
                                                                    $ddd = $this->db->query("select full_name from tbl_users where username ='$username' ")->row_array();
                                                                      if($ddd){?>
                                                                      <?php echo $ddd['full_name'];?>                                                                     <?php }else{?>
                                                                       
                                                                      <?php } ?>
                                                                </td>

                                                                <?php if ($ct['pickup_status'] == '1') { ?>
                                                                    <td><?php echo $ct['status_closed_reason'];?></td>
...
.modal-dialog {
            ...
            <div class="modal-content">
                ...
                <form action="<?php echo base_url(); ?>Pickup_Request_Controller/status_change_pickup" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <input type="hidden" name='status_closed_by' class="form-control" value="<?php echo $username;  ?>" required/>
                        <input type="hidden" name='status' class="form-control" value="<?php echo $approved;  ?>"><br>
                        <input type="text" placeholder="Enter Reason" id="reason" name='status_closed_reason' class="form-control" required/>
                        <span id="error"></span>

                    </div>
                    <button type="submit" name="submit" id="closed_prq12" class="btn btn-success m-2">Status Close</button>
            </div>
            </form>
        </div>
    </div>
    ...
    <div id="show_modal_data1" class="modal fade" role="dialog" style="background: #000;">
        <div class="modal-dialog" style="margin-top: 137px;">
            <div class="modal-content">
                <button type="button" class="btn btn-danger" data-dismiss="modal"><i class="fa fa-times"></i> Close</button>
                <form action="<?php echo base_url(); ?>Pickup_Request_Controller/reschedul_prq" method="post">

                    <div class="modal-body">
                        <?php $approved = 1; ?>
                        <h4 style="margin-top: 50px;text-align: center;">PRQ Reschedule</h4>
                        <label>PRQ Number</label>
                        <input type="text" name="pickup_request_id" class="form-control" id="pickup_request_id1" value="">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
           ...
           $('#closed_prq12').on('submit',function(e){
            ...
            if(reason == ''){
             ...
             $('.reschedul_prq').on('click',function(e){
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#pickup_request_id1').val(response.pickup_request_id); //hold the response in id and show on popup
                        $('#show_modal_data1').modal({
                            backdrop: 'static',
                            keyboard: true,
                            show: true
                        });
        ...
        $('.pickup_boy_assigned').on('click', function(e) {
                var username = $('#username :selected').val()
                var pickup_boy_assigned_date = $('#pickup_boy_assigned_date').val()
               // var pickup_request_id = $('#pickup_request_id').val()
                var allVals = [];
                // console.log(allVals);
                $(".check_assign_prq:checked").each(function() {
                    allVals.push($(this).attr('data-id'));
                });
                    ...
                    if (check == true) {
                        ...
                        $.ajax({
                            ...
                            data: {
                                id: join_selected_values,
                                username: username,
                                pickup_boy_assigned_date: pickup_boy_assigned_date
                                
                            },
                            success: function(data) {
                                console.log(data);
                                $(".pickup_boy_assigned:checked").each(function() {
                                    $(this).parents("tr").remove();
                                });
                                alert("Pickup Boy Assigned.");
                                window.location.reload(true); 
                            },
                            error: function(data) {
                                alert(data.responseText);
