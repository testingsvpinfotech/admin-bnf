...
jQuery(document).ready(function() {
	...
	$('.pickup_request_no').change(function() {
		...
		$.ajax({
			url: "Pickup_Request_Controller/fetch_consigner",
			method: "POST",
			data: {
			...
			success: function(json) {
				...
				//alert(data.customer_type);


        if(data.customer_type == '0' ||  data.customer_type == 'general'){
			...
			function getRate(update){
				...
				if(customer_id != '' && mode_id != '')
				{
					$.ajax({
						type: 'POST',
						url: 'Admin_PRQ_booking/add_new_rate_domestic',
						...
						success: function (data) {	
							...
							if (update) {
								...
								$('#pickup_charges').val(data.pickup_rate);
								$('#delivery_charges').val(0);
			...
			$("#frieht,#transportation_charges,#pickup_charges,#delivery_charges,#courier_charges,#awb_charges,#other_charges,#insurance_charges,#green_tax,#appt_charges,#fuel_charges,#fov_charges").blur(function () {
					...
					if (dispatch_details=='Cash') {
						...
						$.ajax({
							type: 'POST',
							url: 'Admin_PRQ_booking/cashGstCalc',
							data: {
						...
						$.ajax({
							type: 'POST',
							url: 'Admin_PRQ_booking/getFuelcharges',
			...
			$(".chargable_weight,#valumetric_chageable").blur(function () 
			{
				...
				if( mode_id != '')
				{
					$.ajax({
						type: 'POST',
						url: 'Franchise_manager/add_new_rate_franchise_domestic',
						data: 'customer_id=' + customer_id  +'&c_courier_id=' + c_courier_id+'&risk_type=+' +risk_type+'&mode_id=' + mode_id+'&state=' + state +'&city=' + city +'&chargable_weight='+chargable_weight+'&receiver_zone_id='+receiver_zone_id+'&receiver_gstno='+receiver_gstno+'&booking_date='+booking_date+'&invoice_value='+invoice_value+'&dispatch_details='+dispatch_details+'&sender_state='+sender_state+'&sender_city='+sender_city+'&is_appointment='+is_appointment+"&region_id="+region_id+"&doc_type="+doc_type,
			...
			$("#frieht,#transportation_charges,#pickup_charges,#delivery_charges,#courier_charges,#awb_charges,#other_charges,#insurance_charges,#green_tax,#appt_charges").blur(function () {
				// alert('hello');	
					var frieht = parseFloat(($('#frieht').val() != '') ? $('#frieht').val() : 0); 
					var transportation_charges = parseFloat(($('#transportation_charges').val() != '') ? $('#transportation_charges').val() : 0);
					var pickup_charges 	 = parseFloat(($('#pickup_charges').val() != '') ? $('#pickup_charges').val() : 0);
					...
					var totalAmount = (
						frieht + delivery_charges + courier_charges +awb_charges + pickup_charges + other_charges + fov_charges
						);
						// alert(pickup_charges);
					// var totalAmount = (
					// 	frieht + transportation_charges + pickup_charges + delivery_charges + courier_charges +awb_charges + other_charges + fov_charges + insurance_charges + green_tax + appt_charges
					// 	);
						// console.log(totalAmount);
				  ...
				  // 	alert(dispatch_details);
					$.ajax({
						type: 'POST',
						url: 'Franchise_manager/getFuelcharges',
